...
- **Настоящий веб-поиск** — `app/workflows/web_search.py::search_and_answer()`, Tavily (сам фетчит
  страницы результатов — SSRF-защита не нужна, это один доверенный API, не произвольный URL
  студента). Пусто `TAVILY_API_KEY` → честная «не настроено», без похода в сеть — активируется само,
  как только владелец продукта пришлёт ключ, без дополнительных правок кода. Новая `Task.WEB_SEARCH`
...
- **Telegram**: когда бот не находит ответ в материалах, `CONSENT_KEYBOARD` (`bot/handlers/query.py`)
  теперь предлагает не только «общие знания ИИ», но и «🌐 Найти в интернете» / «🔬 Искать в PubMed» —
  естественная точка входа (бот сам предлагает искать, когда не знает — «работать как обычный ИИ»,
  формулировка пользователя), не отдельная команда для этого. Плюс новая команда **`/selfcheck`**
