...
- **Батч 6 (готов, 4A закрыт целиком):** 4A.7 голос. `app/llm/provider.py::transcribe()` — Whisper
  ...
  Telegram-поверхности сознательно нет — тот же прецедент, что у evaluate-эндпоинтов из батча 2
  (оценочные workflow API-only, естественный сценарий — сайт с записью, а не команда в чате).
