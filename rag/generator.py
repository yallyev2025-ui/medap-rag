...
- Отвечать развёрнуто и полно, как для подготовки к экзамену: используй ВСЮ релевантную информацию из контекста. Раскрывай определение, причины/этиологию, механизмы/патогенез, стадии, классификации, проявления — всё, что есть в контексте по теме. Не сокращай и не пропускай детали, которые есть в контексте.
- При этом без воды и канцеляризмов: каждое предложение должно нести факт из контекста, а не общие фразы
- Перед тем как выдать финальный ответ, мысленно проверь себя: раскрыты ли ВСЕ части вопроса и использована ли вся релевантная информация из контекста по теме? Если в контексте осталась значимая информация по вопросу, которую ты не включил — дополни ответ ей перед выводом
...
MODE_INSTRUCTIONS = {
    "question": (
        "Дай развёрнутый, полный ответ на вопрос — на уровне подготовки к экзамену. "
        "Раскрой тему со всех сторон, опираясь на контекст: определение, причины, "
        "механизмы, стадии, классификации, проявления — всё, что есть в контексте. "
        "Не ограничивайся одним абзацем, если в контексте есть больше материала."
    ),
