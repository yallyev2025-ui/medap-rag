...
и реранкер. По умолчанию стоит топовый по качеству `BAAI/bge-reranker-v2-m3`
(≈2.3 ГБ весов) — установившийся размер процесса ориентировочно **6–7 ГБ RAM**,
а пики при реранке могут подходить к 8 ГБ.

**На лимите 8 ГБ это впритык.** Чтобы влезть: реранк идёт малыми батчами
(`RERANK_BATCH_SIZE=8`, снижает пик памяти). Если всё же ловишь OOM в логах —
лучший вариант поднять лимит RAM сервиса (Railway Pro, например 16 ГБ).

Быстрые откаты без редеплоя кода (через env vars):
- лёгкий реранкер: `RERANKER_MODEL_NAME=cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`
  (≈0.5 ГБ, общий потребление падает до **4–5 ГБ**, качество реранка чуть ниже);
- уменьшить `RERANK_BATCH_SIZE` (например до `4`) — ещё ниже пик памяти;
