...
и реранкер. По умолчанию стоит лёгкий `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`
(≈0.5 ГБ) — суммарное потребление процесса ориентировочно **4–5 ГБ RAM**, спокойно
влезает в лимит 8 ГБ.

Для максимального качества реранка (если RAM сервиса ≥16 ГБ) можно включить топовый
`RERANKER_MODEL_NAME=BAAI/bge-reranker-v2-m3` (≈2.3 ГБ весов) — тогда ориентир
**6–7 ГБ**, на 8 ГБ это впритык и рискует OOM, поэтому только с запасом по RAM.

Если в логах появляется OOM:
- уменьшить `RERANK_BATCH_SIZE` (например до `4`) — ниже пик памяти;
- остаться на лёгком реранкере (значение по умолчанию);
