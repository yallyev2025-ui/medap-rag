...
и реранкер. С лёгким реранкером по умолчанию (`mmarco-mMiniLMv2-L12`, ≈0.5 ГБ) процесс
бота потребляет ориентировочно **4–5 ГБ RAM**. Перед запуском проверить лимит RAM
сервиса в Railway (Settings → Resources).

Для максимального качества реранка можно поставить `RERANKER_MODEL_NAME=BAAI/bge-reranker-v2-m3`
(≈2.3 ГБ весов) — тогда ориентир **6–7 ГБ RAM**, может потребоваться план с большим лимитом.

Если в логах появляется OOM:
- увеличить лимит RAM сервиса (Railway Pro), либо
- оставить лёгкий реранкер (значение по умолчанию), либо
